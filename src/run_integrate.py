# run.py integrated wihtth RAG and follow-up questions in cli
import asyncio
import os
...
from .ai.providers import get_model, trim_prompt
...
async def run(query: str, intent, rag_results: List[Dict]):
    ...
    # add RAG to initial query 
    # Initialize helper for formatting queries
    helper = HelperQuery(max_chunks_per_query=20)
    initial_query = helper._format_enhanced_query(
        query=initial_query,
